...
function Dashboard() {
  ...
  const handleData = (type) => {
    if (type == "Users") {
      ...
      axios.get("http://127.0.0.1:8001/api/cars").then((response) => {
        setHeader([
          "id",
          "brand",
          "model",
      ...
      axios.get("http://127.0.0.1:8001/api/rents").then((response) => {
        setHeader([
          "id",
          "rental_date",
          "return_date",
  ...
  const handleUpdateItem = (itemId, updatedItem) => {
    const endpoint = `http://127.0.0.1:8001/api/${type}/${itemId}`;
  ...
  const handleDelete = (id) => {
    const endpoint = `http://127.0.0.1:8001/api/${type}/${id}`;
